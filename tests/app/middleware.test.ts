...
/**
 * `@/middleware` imports `@/lib/auth`, which imports `@/lib/db` — a real
...
const { config, decide, default: middleware } = await import('@/middleware')
...
describe('the real auth()-wrapped handler (src/middleware.ts default export)', () => {
